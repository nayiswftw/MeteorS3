...
static String buildWeatherUrl() {
    ...
    url += "?latitude=" + String(config::LATITUDE, 6);
    url += "&longitude=" + String(config::LONGITUDE, 6);
...
void apiService() {
    ...
    if (!w.valid || (millis() - state::lastWeatherUpdate() >= config::WEATHER_INTERVAL_MS)) {
        refreshWeather();
    }

    const AirData& a = state::air();
    if (!a.valid || (millis() - state::lastAirUpdate() >= config::AIR_INTERVAL_MS)) {
        refreshAir();
    }
}
